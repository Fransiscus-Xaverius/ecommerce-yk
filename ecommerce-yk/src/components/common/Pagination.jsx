  ...
  return (
    <div className={`flex items-center justify-center ${className}`.trim()}>
      <div className="flex items-center space-x-0.5 sm:space-x-1">
        ...
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="sm:h-3 sm:w-3 md:h-3.5 md:w-3.5 lg:h-4 lg:w-4"><polyline points="15,18 9,12 15,6"></polyline></svg>
        </button>
        {pages.map((p, i) => p === '...' ? (
          <span key={i} className="flex h-7 w-7 items-center justify-center text-xs text-gray-500 sm:h-8 sm:w-8 md:h-9 md:w-9 lg:h-10 lg:w-10 lg:text-sm">...</span>
        ) : (
          <button
            key={p}
            onClick={() => setPage(p)}
        ...
        >
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="sm:h-3 sm:w-3 md:h-3.5 md:w-3.5 lg:h-4 lg:w-4"><polyline points="9,18 15,12 9,6"></polyline></svg>
        </button>
      </div>
    </div>
  );
